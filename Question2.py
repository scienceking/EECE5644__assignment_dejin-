...
# 3. Select the best number of components for GMM using cross-validation
def select_best_gmm(features, max_components=10):
    best_gmm = None
    best_score = -np.inf
    for n_components in range(1, max_components + 1):
        ...
        for train_index, val_index in kf.split(features):
            ...
            cv_scores.append(gmm.score(val_features))

        avg_score = np.mean(cv_scores)
        print(f"Components: {n_components}, CV Log-Likelihood: {avg_score:.4f}")

        if avg_score > best_score:
            best_score = avg_score
            best_gmm = gmm

    print(f"Best Components: {best_gmm.n_components}, Best CV Log-Likelihood: {best_score:.4f}")
    return best_gmm
...
# 5. Display the original image and segmentation result
def display_segmentation(original_image, labels, n_components):
    ...
    # Display the original image and segmentation
...
# Extract, normalize features, and select the best GMM
features = extract_features(image)
normalized_features = normalize_features(features)
best_gmm = select_best_gmm(normalized_features)

# Assign labels to each pixel and display the results
labels = assign_labels(best_gmm, normalized_features)
display_segmentation(image, labels, best_gmm.n_components)
